...
public class Main {
     ...
     * A1: actually, why wouldn't we use a switch case? There are only 4 actions, and they are different each other. Thoose actions aren't consequencial, so loops shouldn't work well.
     *
     * Q2: We have a list of 1000 users to send emails to
     * A2: for each, the best way to do one specific action to a list (so an array) of users
     *
     * Q3: We want to print out the first 80 customer names from our bank database
     * A3:fori should go well. Aren't many differences with while actually. (in this case)
     *
     * Q4: We want to read a file 100 lines at a time, without loading the full file into our program
     * A4:I would use while, but as I said, I do not notice many differences with while loop so...
     */
}
